...
savefig('.\net\VGG16_design\confusion_matrix2x2.fig')
...
savefig('.\net\VGG16_design\confusion_matrix.fig')
...
figure
subplot(211);
plot(trainACC,  'LineWidth', 2);
hold on
plot(valACC, 'r.', 'MarkerSize', 20);
axis([0 length(trainACC) 0 105])

subplot(212);
plot(trainLoss, 'LineWidth', 2);
hold on
plot(valLoss, 'r.-', 'MarkerSize', 20);

axis([0 length(trainACC) -1 max(trainLoss)+1])
savefig('.\net\VGG16_design\training_progress.fig')
